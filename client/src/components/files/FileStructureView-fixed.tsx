...
function FileStructureView() {
    ...
    const handleCreateRootDirectory = () => {
        console.log("Creating new root directory");
        // Cancel any existing creation first
        setCreatingNewItem(null);
        // Then set the new creation state in the next tick
        setTimeout(() => {
...
function Directory({
    ...
    // We'll use this to track if the directory has been initialized by a click
    const [hasBeenClicked, setHasBeenClicked] = useState<boolean>(false)
    
    // Initialize the directory's open state if needed for newly created folders
    useEffect(() => {
        // If isOpen is undefined, ensure it's set to a boolean value
        if (item.isOpen === undefined && !hasBeenClicked) {
    ...
    return (
        <div className="overflow-x-auto">
            <div className="flex w-full justify-between items-center rounded-md px-2 py-1 hover:bg-darkHover group">
                <div 
                ...
                {/* Quick action buttons - visible on hover - no conditional rendering */}
