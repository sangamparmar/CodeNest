...
const CodeEditor = () => {
  ...
  const [activityLog, setActivityLog] = useState<Array<{ message: string; type: string }>>([])

  // Function to start collaboration
  const startCollaboration = () => {
    setCollaborationStarted(true)

    // Simulate activity logs with a sequence of events
    const simulateActivity = () => {
      ...
      const interval = setInterval(() => {
        if (i < activities.length) {
          ...
          // When the last activity is added, immediately close the welcome screen
          if (i === activities.length) {
            ...
            <div className="max-w-lg w-full">
              {!collaborationStarted ? (
                ...
                <div className="bg-black/60 border border-violet-500/30 rounded-lg p-6">
                  ...
                  <div className="space-y-3 mb-4 h-64 overflow-y-auto font-mono text-sm">
