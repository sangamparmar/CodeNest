...
const Register: React.FC = () => {
  ...
  // Terminal typing effect with cleanup
  useEffect(() => {
    if (statusMessage && terminalRef.current) {
      ...
      const speed = 5 // Significantly faster typing speed
      ...
      // Cleanup function to clear timers if component unmounts or status changes
      return () => {
        if (typingTimer) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      // Simulate network delay for visual effect - reduced delay
